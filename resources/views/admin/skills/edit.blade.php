...
@section('header_title', 'Edit Skill')

@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-xl font-bold text-slate-800">Edit Skill: {{ $skill->name }}</h2>
        <p class="text-sm text-slate-500 mt-1">Update the details of your skill.</p>
    </div>
...
<div class="bg-white shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-100 rounded-2xl overflow-hidden p-6 md:p-8 max-w-2xl">
    <form action="{{ route('admin.skills.update', $skill) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all sm:text-sm" id="name" name="name" type="text" value="{{ old('name', $skill->name) }}" required>
